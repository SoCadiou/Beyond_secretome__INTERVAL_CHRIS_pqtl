...
# install.packages("ggraph","readr","dplyr","ggplot2","scales","ggrepel","Matrix","ggraph","tidygraph","data.table")
#}
...
################################################
colocalization_results_filtered_path ="../results/Colocalization_results_filtered.csv"
hotspots_df_path = "../results/Hotspots_df.csv"
LB_results_path = "../results/LB_in_hotsposts.csv"
type_of_clustering = "connected" # either "greedy", "connected"
mapping_path<-"../data/supplementary_table_1.xlsx" #excel file - provided as supplementary table 1 of corresponding paper
##############################################
...
LB_in_hotsposts$start<- sub("^[^_]*_([^_]*)_.*$", "\\1",LB_in_hotsposts$locus_START_END_37)
LB_in_hotsposts$end  <- sub("^.*_", "", LB_in_hotsposts$locus_START_END_37)
LB_in_hotsposts$start <- as.numeric(LB_in_hotsposts$start)
LB_in_hotsposts$end <- as.numeric(LB_in_hotsposts$end)
...
filename <- paste("../results/",foldname, "/Node_legend.txt", sep="")
...
communities <-  full_join(LB_in_hotspost, communities, by= c("CHR"="chr", "start"="start","end"="end", "SeqID"="phenotype_id"))
...
filename <- paste("../results/",foldname, "/Hotspot_communities_",type_of_clustering,".csv", sep="")
...
plot_name <- paste("../results/",foldname, "/Plots_graph_",type_of_clustering,".jpg", sep="")
...
ggplot(degree_df, aes(x = Degree)) +
  ...
  #theme_minimal()+
  theme_bw()
plot_name <- paste("../results/",foldname, "/Degree_plot_",type_of_clustering, ".png", sep="")
ggsave(plot_name, width = 25, height = 20, units = "cm")



mapping<-read_excel(mapping_path, sheet = "ST1", skip = 1)
mapping <- mapping[, c("chromosome","SeqID","TSS", "UniProt_ID") ]
colnames(mapping) <- c("phenotype_chr","phenotype_id","phenotype_pos","UniProt_ID")

communities <- left_join(communities, mapping, by=c("SeqID"="phenotype_id", "UniProt_ID"="UniProt_ID"), relationship = "many-to-many")
...
save(communities, file=paste("../results/",foldname, "/communities_regional_plot_",type_of_clustering, ".RData", sep="") )

communities_regional_plot <- communities %>%
  select(POS_37, bp_cum_map, group, cis_or_trans, UniProt_ID)

ggplot(communities_regional_plot, aes(x = POS_37, y = bp_cum_map, color = as.factor(group), shape = cis_or_trans)) +
  geom_point(size = 3) + 
  geom_text_repel(
...
plot_name <- paste("../results/",foldname, "/Regional_plot_",type_of_clustering,".png", sep="")
ggsave(plot_name, width = 25, height = 20, units = "cm")

cat('Compute genomics distance \n')
...
filename <- paste("../results/",foldname, "/Gen_distance_communities_", type_of_clustering,".csv", sep="")
write.csv(dist_matrix, file = filename)

communities_to_retain <- communities_regional_plot %>%
  group_by(group) %>%
  summarise(n = n()) %>%
  filter(n > 4)
...
communities_res <- communities_regional_plot %>%
  filter(group %in% communities_to_retain$group)

ggplot(communities_res, aes(x = POS_37, y = bp_cum_map, color = as.factor(group), shape = cis_or_trans)) +
  geom_point(size = 3) +  
  geom_text_repel(
  ...
  #theme_minimal() + 
  theme_bw()
plot_name <- paste("../results/",foldname, "/Regional_plot_",type_of_clustering,"_restrict_to_large_communities.png", sep="")
ggsave(plot_name, width = 25, height = 20, units = "cm")
...
ggsave(p, filename=paste("../results/",foldname, "/HighQuality_Network_",type_of_clustering, ".png", sep=""), width=8, height=8, dpi=300)
...
ggsave(p, filename=paste0("../results/",foldname, "/HighQuality_Network_investigation_",type_of_clustering,".png", sep=""), width=8, height=8, dpi=300)
